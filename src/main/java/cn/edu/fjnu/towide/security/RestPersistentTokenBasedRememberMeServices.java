...
public class RestPersistentTokenBasedRememberMeServices extends PersistentTokenBasedRememberMeServices {
	...
	protected void setCookie(String[] tokens, int maxAge, HttpServletRequest request, HttpServletResponse response) {
		...
		response.setHeader("remember-me", encodeCookie(tokens));
...
//		super.logout(request, response, authentication);
	...
	protected String extractRememberMeCookie(HttpServletRequest request) {
		...
		RequestHead requestHead=HttpServletRequestUtil.getRequestHead(request);
		if (requestHead==null) {
			return null;
		}
		return requestHead.getToken();
/*		JSONObject jsonObject=JsonUtils.getJsonFromRequest(request);
		...
		return token;
*///		return request.getHeader("remember-me");
	}
	/**
...
//		System.out.println(request.getMethod());
		...
		if (!param.containsKey("rememberMe")) {
			return false;
		}
		rememberMe=param.getString("rememberMe");
		if (rememberMe.equals("yes")) {
			return true;
		}
		return false;
		
/*		String remember_me=request.getHeader(parameter);
		if (remember_me==null) {
			return false;
		}
